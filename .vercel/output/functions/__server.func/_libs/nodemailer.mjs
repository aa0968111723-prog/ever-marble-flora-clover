...
function encodeWord(data, mimeWordEncoding, maxLength) {
	...
	if (mimeWordEncoding === "Q") encodedStr = encode(data).replace(/[^a-z0-9!*+\-/=]/gi, (chr) => {
		...
		encodedStr = typeof data === "string" ? data : encode$1(data);
...
function _handleAddress(tokens, depth) {
	...
	for (let i = 0, len = tokens.length; i < len; i++) {
		...
		if (token.type === "operator") switch (token.value) {
			...
			if (prevToken && prevToken.noBreak && parts.length && (prevToken.value !== ")" || parts[parts.length - 1].slice(-1) === "@" || token.value.charAt(0) === "@")) {
...
var nodemailer_exports = /* @__PURE__ */ __exportAll({ createTransport: () => createTransport });
(process.env.ETHEREAL_API || "https://api.nodemailer.com").replace(/\/+$/, "");
(process.env.ETHEREAL_WEB || "https://ethereal.email").replace(/\/+$/, "");
(process.env.ETHEREAL_API_KEY || "").replace(/\s*/g, "");
[
	"true",
	"yes",
	"y",
...
//#endregion
export { nodemailer_exports as t };
